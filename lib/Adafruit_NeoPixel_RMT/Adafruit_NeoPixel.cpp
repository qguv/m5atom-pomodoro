...
#define MAX_LEDS 768
#define MAX_BITS MAX_LEDS * 8

// Each tick is 50 ns long (4 clock cycles, at 80 MHz)
#define TICK_SPEED 50

// See https://wp.josh.com/2014/05/13/ws2812-neopixels-are-not-so-finicky-once-you-get-to-know-them/
#define T0H       400 / TICK_SPEED
#define T0L       650 / TICK_SPEED
#define T1H       850 / TICK_SPEED
#define T1L       450 / TICK_SPEED
#define TRES     6000 / TICK_SPEED

rmt_data_t signal_data[MAX_BITS + 1];
rmt_obj_t* rmt_send = NULL;

// Originally based on https://github.com/espressif/arduino-esp32/blob/master/libraries/ESP32/examples/RMT/RMTWriteNeoPixel/RMTWriteNeoPixel.ino
float ws2812_init(int txpin) {
    if ((rmt_send = rmtInit(txpin, true, RMT_MEM_64)) == 0) {
        return -1.0;
    }

    return rmtSetTick(rmt_send, TICK_SPEED);
}

void ws2812_write(uint32_t bytes, uint8_t *pixel_data) {
    if (bytes > MAX_LEDS) return;

    int i = 0;
    for (int led = 0; led < bytes; led++) {
        for (int bit = 0; bit < 8; bit++) {

            // For each bit of each pixel, setup a on or off signal

            uint8_t mask = 1 << (8 - bit);
            uint8_t on   = pixel_data[led] & mask;

            // Turn a bit on
            if (on) {
                signal_data[i].duration0 = T1H;
                signal_data[i].level0 = 1;
                signal_data[i].duration1 = T1L;
                signal_data[i].level1 = 0;
                // Serial.print("#");
            }
            
            // Turn a bit off
            else {
                signal_data[i].duration0 = T0H;
                signal_data[i].level0 = 1;
                signal_data[i].duration1 = T0L;
                signal_data[i].level1 = 0;
                // Serial.print(" ");
            }
            ...
            // Move on to the next bit signal
            i++;
        }
    }

    // Send the reset signal, which latches the data and causes display
    signal_data[i].duration0 = TRES;
    signal_data[i].level0    = 0;
    signal_data[i].duration1 = 0;
    signal_data[i].level1    = 0;

    // Serial.print("rmtWrite bits ");
    // Serial.println(bytes * 8);

    // Send the data
    rmtWrite(rmt_send, signal_data, bytes * 8 + 1);
}
...
void espShow(
    ...
    // Serial.print("NEOPIXELING ");
    // Serial.print(numBytes);
    // Serial.println(" BYTES");

    ws2812_write(numBytes, pixels);
}
